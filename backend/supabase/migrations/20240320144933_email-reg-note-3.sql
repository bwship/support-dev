DROP POLICY IF EXISTS email_registration_update_policy ON public.email_registration;
...
USING (true) -- Deny INSERT and DELETE operations
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.email_registration WHERE email = _email) THEN
    ...
    INSERT INTO public.email_registration (email, is_approved, created_by, notes)
