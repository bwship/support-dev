...
const removeClicked = async () => {
  ...
  try {
    ...
    addToast({ text: t('Helper removed successfully'), variant: 'success' })
    ...
    console.error('Deleting helper failed:', error.message)
...
<template>
  <v-dialog v-model="showDialog" width="500" persistent>
    ...
    <v-card>
      ...
      <v-divider />
      <v-card-text>{{ $t('Are you sure want to remove this helper?') }} </v-card-text>
